  ...
  static delay = (ms, acc, msg, obj) => {
    return new Promise(async (resolve) => {
      ...
      const interval = setInterval(async () => {
        ...
        if (acc != undefined) {
          await twist.log(msg, acc, obj, `Delaying for ${this.msToTime(remainingMilliseconds)}`);
        } else {
    ...
    for (const abi of ABIs) {
      for (const item of abi) {
        if (item.type === "function") {
          ...
          const selector = "0x" + ethers.keccak256(ethers.toUtf8Bytes(functionSignature)).slice(0, 8);
