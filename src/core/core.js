...
export default class Core {
  ...
  async connectWallet() {
    try {
      ...
      await Helper.delay(1000, this.acc, `Connecting to Account : ${accIdx + 1}`, this);
      ...
      await Helper.delay(1000, this.acc, `Wallet connected ${JSON.stringify(this.wallet.address)}`, this);
    } catch (error) {
      throw error;
    }
  }
  ...
  async getBalance(update = false) {
    try {
      if (!update) {
        await Helper.delay(500, this.acc, `Getting Wallet Balance of ${this.wallet.address}`, this);
      }

      const ethBalance = ethers.formatEther(await this.provider.getBalance(this.wallet.address));
  ...
  async getLastGm() {
    try {
      ...
      const contract = new ethers.Contract(DAILYGM.CA, DAILYGM.ABI, this.wallet);
  ...
  async getOptimalNonce() {
    try {
      ...
      const optimalNonce = pendingNonce > latestNonce ? pendingNonce : latestNonce;
      return optimalNonce;
    } catch (error) {
      throw error;
    }
  ...
  async estimateGasWithRetry(address, amount, rawdata, directThrow = false, retries = 3, delay = 3000) {
    for (let attempt = 0; attempt < retries; attempt++) {
      try {
        ...
        if (directThrow) {
          ...
          await Helper.delay(delay, this.acc, `${err.reason}... Attempt ${attempt + 1} of ${retries}`, this);
  ...
  async buildTxBody(data, estimateGas = true, direct = false, value = 0) {
    ...
    let gasLimit, tx;
    if (estimateGas) {
      gasLimit = await this.estimateGasWithRetry(data.to, value, data.data, direct);
      tx = {
        to: data.to,
        from: this.address,
        value: value,
